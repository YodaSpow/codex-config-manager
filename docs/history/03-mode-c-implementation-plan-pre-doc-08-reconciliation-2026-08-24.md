...
**Reconciled:** 21 August 2026 with Doc 5 deterministic managed-state publication history  
...
Mode C must read all seven governing documents completely before changing the repository or any external state:
...
7. This Doc 3 — the reconciled Mode C sequence, current Mac Studio evidence, proof gates, stop conditions and handoff boundary.
...
- Docs 1, 2, 4 and 5 refine implementation contracts using verified repository, machine and publication-history reality;
...
The protected pre-reconciliation Doc 3 snapshot linked above is a byte-identical drift reference, not an eighth governing specification and not the active implementation plan. It must remain unchanged. If this active plan is revised again before implementation, compare it with that snapshot so unrelated requirements are not silently lost; do not copy superseded text back over explicit later reconciliations.
...
└── skills/                  managed user-skill payload only
            │
            │ validated staged diff → deterministic component summary
...
│   ├── history/
│   │   └── 03-mode-c-implementation-plan-pre-doc-05-reconciliation-2026-08-21.md
...
`<skill-name>` is always the dynamic immediate child of `skills/`, never an allowlisted current name and never a nested filename such as `SKILL.md`. Phase 6 defines this topology mapping but does not infer Git actions or render commit prose; Phase 9 classifies the validated staged before/after trees.
...
- every publishable staged path maps unambiguously to `AGENTS.md` or one dynamic top-level user-skill component.
...
**Gate 11:** The exact repository test command passes with a decisive summary; targeted safety and deterministic-publication suites pass independently; temporary artifacts are contained and removed; no live Codex or launchd state was touched by simulation.
