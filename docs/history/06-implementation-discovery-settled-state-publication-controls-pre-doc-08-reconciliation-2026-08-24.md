...
**Status:** Operator-approved timing and publication-control direction; not yet reconciled into Doc 3 or implemented  
...
**Relationship to existing documents:** This record refines the publisher timing and configuration contract and defines the eligibility layer immediately before Doc 5 managed-state publication. It does not replace or modify Docs 3 or 5.  
**Related documents:** [Doc 3 — Mode C Implementation Plan](03-mode-c-implementation-plan.md) · [Doc 5 — Deterministic Managed-State Publication History](05-implementation-discovery-deterministic-managed-state-publication-history.md)
...
- ▶ Exact scheduled-mode edge behaviour and which optional modes enter the first implementation remain explicit decisions before Doc 3 reconciliation.
...
Doc 3 therefore requires a future surgical reconciliation before implementation. This document records the new truth without performing that reconciliation.
...
`check_interval` must be greater than zero. `minimum_interval` must be greater than zero when throttled mode is selected. A future `settle_period: 0s` may represent disabled settling/immediate eligibility, but that value must not bypass source validation, staging validation or Git safety.
...
The pending-state record is operational metadata, not managed payload. It must never enter `source/staging/` or Git.

Before publication, the existing authoritative rsync dry-run, real bounded rsync, staging validation and post-sync equivalence checks still apply. If the source changes during ingestion or validation, publication fails safely and the new source state must settle again. Settlement never replaces the existing proof pipeline.
...
publication-mode eligibility satisfied
        ↓
authoritative rsync/staging/Git validation succeeds
        ↓
...
- mutate staging while a source state is merely pending or unsettled;
- publish an unsettled source;
- weaken `.system/**` or `.DS_Store` exclusions;
- bypass staging equivalence or Git path validation;
- parse or infer managed-content meaning;
- create a no-op commit;
- turn a routine publication into a SemVer release.

`source/staging/` remains the persistent last-published managed mirror until a state is both settled and mode-eligible. This avoids allowing a pause, schedule or throttle to leave unpublished authoring state sitting in tracked staging.
...
Mode-specific keys must be validated conditionally. Missing required keys, unknown modes or keys that conflict with the selected mode must stop before staging or Git mutation.
...
This is the default and preferred steady-state mode. “After settle” means no additional cadence delay is imposed after the configured quiet period. Publication still includes every existing environment, role, path, source, staging and Git validation gate.
...
- perform real rsync into staging;
- mutate staging;
...
14:30–17:59  remain pending without staging mutation
...
| `paused` | Remain pending; never mutate staging or Git | Operator-controlled hold | Temporary publication freeze |
...
- any invalid or unknown mode fails closed before staging or Git mutation;
...
- changing a scheduled time or throttle interval recalculates eligibility without rewriting managed source or staging;
- a configuration edit is local operational control and must not itself become managed staged content;
...
observe → settle → apply publication mode → become eligible
                                           ↓
existing rsync/staging validation
                                           ↓
Doc 5
...
## Impact assessment for Doc 3

Doc 6 has cross-cutting impact but one primary orchestration home. A future Doc 3 reconciliation should account for the following without restructuring unrelated phases.
...
| Locked architecture | Insert observation, pending settlement and mode eligibility before real staging mutation |
...
| Phase 7 | Preserve staging as the last-published mirror; pending/unsettled/mode-held state must not enter staging |
| Phase 8 | Make the publisher a deterministic state machine: observe, track fingerprint, settle, apply mode, then run rsync/staging/Git orchestration |
...
| Phase 14 | Audit that held state never mutates staging/Git and that configured modes cannot weaken exclusions or publication safety |
...
## Safe future Doc 3 reconciliation

No Doc 3 edit is authorised by this record. If the operator later requests reconciliation, the safe Mode B workflow is:
...
6. keep Docs 5 and 6 unchanged unless a separately identified contradiction genuinely requires their revision;
...
- ✅ The duration grammar, authoring boundary, staging invariant, Doc 5 relationship and Doc 3 impact surface are captured.
- ▶ A later explicitly authorised Mode B may reconcile this record into Doc 3 using a new protected snapshot and drift audit.
- ⛔ Docs 3 and 5 remain unchanged by this record.
