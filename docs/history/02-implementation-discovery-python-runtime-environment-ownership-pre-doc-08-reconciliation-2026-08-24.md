...
**Related discovery:** [Doc 1 — Implementation Discovery — Managed Skill Ingestion Exclusions](01-implementation-discovery-managed-skill-ingestion-exclusions.md)
...
- the existing persistent staging and source-of-truth model.
...
`source/` and `src/` have deliberately different meanings:

```text
source/staging/     persistent Git-tracked managed payload
...
| --- | --- |
| `publisher.py` | Studio preflight, live-to-staging comparison, sync, validation and scoped publication orchestration |
| `consumer.py` | Git update, staging validation, staging-to-live comparison, deployment and verification orchestration |
...
| `validation.py` | Environment, staging, deployment and post-operation proofs shared where genuinely common |
...
The repository tracks the complete reusable system, not only staging. Automated publisher commits remain more narrowly bounded than public project contents.
...
It must not independently redefine publisher behaviour, shared architecture, staging semantics, managed scope or environment ownership.
...
9. Establish persistent staging using the Doc 1 ingestion boundary.
...
- ⛔ No code, dependency state, `.venv`, `.tools`, staging payload, Git bootstrap or launchd service is created by this document.
