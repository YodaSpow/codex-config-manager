...
- the deterministic machine-identity discovery;
- the deterministic managed-state publication-history discovery;
...
Mode C must read all ten governing documents completely before changing the repository or any external state:
...
6. [Doc 5 — Deterministic Managed-State Publication History](05-implementation-discovery-deterministic-managed-state-publication-history.md) — headless GitHub visibility, staged-diff component semantics, publication receipts and deliberate release separation.
7. [Doc 6 — Settled-State Publication Controls](06-implementation-discovery-settled-state-publication-controls.md) — recursive fingerprint settlement, publication modes and mutation eligibility.
8. [Doc 7 — Portable Global Guidance and Per-Skill ZIP Distribution](07-implementation-discovery-portable-skills-zip-distribution.md) — deterministic download artifacts, README reconciliation and human transfer.
9. [Doc 8 — Root-Level Latest Managed Snapshot](08-architecture-reconciliation-root-level-latest-managed-snapshot.md) — root-level `latest/`, private candidate separation and the reconciled implementation/verification overlay.
10. This Doc 3 — the reconciled Mode C sequence, current Mac Studio evidence, proof gates, stop conditions and handoff boundary.
...
- Docs 1, 2 and 4–8 refine implementation contracts using verified repository, machine, publication, settlement, distribution and topology reality;
...
The protected Doc 3 snapshots linked above are byte-identical drift references, not governing specifications and not the active implementation plan. They must remain unchanged. If this active plan is revised again before implementation, compare it with the immediate pre-edit snapshot so unrelated requirements are not silently lost; do not copy superseded text back over explicit later reconciliations.
...
- `.venv`, `.tools`, `pyproject.toml`, application code, tests, config, private candidate handling, `latest/`, `upload-ready/` and launchd implementation do not yet exist;
...
└── skills/
    ...
    └── every current/future user skill, discovered dynamically
            │
            │ settle/mode eligibility + private candidate + exact repository-owned rsync
            ▼
latest/
├── AGENTS.md                present only when present in authoritative source
└── skills/                  managed user-skill payload only
            │
            │ deterministic artifacts + validated Git-staged diff
            ▼
upload-ready/ + bounded README download section
            │
            │ deterministic component summary
...
The Mac Studio live managed state is authoritative. A private temporary candidate is validated before root-level `latest/` changes. `latest/` is the persistent Git-tracked canonical repository snapshot, never an authoring source on the Mac Studio. `upload-ready/` is derived only from validated `latest/`. GitHub is the canonical shared published repository state. The Mac mini validates and consumes `latest/` and never depends on a live mount, hard link, symlink to, or network connection with the Mac Studio.
...
- private-candidate and `latest/` validation;
- Git diff, staging and publication;
- consumer `latest/` validation;
- consumer deployment;
- automatic managed deletion;
- no-op detection;
- tests and simulations.

`.system/**` must never become private candidate or canonical `latest/` state. If a manually or externally introduced `.system` path is detected in `latest/`, validation must fail safely and report it; Codex Config Manager must not deploy, publish, clean or delete it.
...
- if the configured `/Users/spowart/.codex` root is missing, unreadable, inaccessible, outside the accepted containment boundary or affected by an I/O/permission failure, the publisher stops without changing a candidate, `latest/`, artifacts, README or Git;
...
- the managed `latest/skills/` container may remain as structural representation even when it contains no managed user skills.
...
managed item absent from readable authoritative Mac Studio source
        ↓
bounded deletion from latest
        ↓
Git records deletion
        ↓
GitHub publishes deletion
        ↓
Mac mini receives deletion in `latest/`
        ↓
...
- persistent `latest/` managed state;
...
GitHub is the initial human-facing portal for this headless system: commit history supplies the managed-state timeline, exact diffs supply forensic detail, repository documentation supplies context, and deliberate tags/Releases identify project milestones. Routine managed-state publications use commit SHAs and deterministic component-level messages; they must never create automatic version tags. Normal human/AI project-development commits, unattended Mac Studio managed-state publications and explicitly approved Codex Config Manager releases are separate Git activities.

The Mac Studio owns the initial repository architecture, publisher implementation, shared contracts, initial consumer implementation, consumer simulation, permanent documentation and bounded handoff. The Mac mini owns truthful real-consumer validation and consumer-specific refinements within that established contract. The Mac mini must not silently redefine publisher behaviour, source-of-truth direction, `latest/`, private candidate handling, scope, deletion, public config or shared architecture.
...
│   ├── 05-implementation-discovery-deterministic-managed-state-publication-history.md
│   ├── 06-implementation-discovery-settled-state-publication-controls.md
│   ├── 07-implementation-discovery-portable-skills-zip-distribution.md
│   ├── 08-architecture-reconciliation-root-level-latest-managed-snapshot.md
│   ├── history/
│   │   └── <protected documentation snapshots>
...
`latest/` is transported state. `src/` is application code. `tooling/rsync/` is the tracked reproducibility contract. `.tools/rsync/` is the generated local runtime. `publication.py` represents the conceptual managed-change-set and message/receipt projection boundary; Mode C may merge it into `git.py` or another cohesive module if that produces a clearer implementation. Mode C may consolidate very small modules where that reduces empty abstraction, but it must preserve the ownership boundaries and separately installed publisher and consumer entry points.
...
1. read the seven governing documents completely;
...
5. record hashes for all seven governing documents and verify that the protected pre-reconciliation Doc 3 snapshot still has SHA-256 `182e470f0e79d29ff91bc9e14efd5681bf82f4836236db390c3a46241b2ed4ee`;
...
- Codex root;
- repository root;
- repository root, from which locked `latest/` and `upload-ready/` paths are derived;
- runtime-state and lock locations;
- log locations;
- publisher `check_interval: 1m`, `settle_period: 5m` and `publication.mode: after_settle` as the preferred initial Doc 6 configuration;
- validated optional `paused`, `scheduled` and `throttled` publication-mode settings under Doc 6;
- a separately configured consumer interval, initially `300` seconds;
...
The same centralized scope contract must expose the stable component vocabulary later consumed by Git publication:

```text
latest/AGENTS.md                 → AGENTS.md
latest/skills/<skill-name>/**   → <skill-name>
```

`<skill-name>` is always the dynamic immediate child of `skills/`, never an allowlisted current name and never a nested filename such as `SKILL.md`. Phase 6 defines this topology mapping but does not infer Git actions or render commit prose; Phase 9 classifies the validated Git-index before/after trees.

**Gate 6:** Publisher and consumer derive their inverse mappings and stable component identities from one tested managed-scope contract; all exclusion/deletion options are identical where required; dry-run is non-mutating; a second dry-run proves no meaningful difference.

## Phase 7 — Establish private candidate handling and validate root-level `latest/`

Only after the environment and scope gates pass may Mode C create a private per-transaction candidate and promote its validated managed payload to:
...
Initial ingestion must originate only from the readable authoritative Mac Studio managed source. It must not manually copy hard-coded current skills. It must not place the transported global `AGENTS.md` at repository root. The candidate must be private, ignored or system-temporary, unique to one transaction and disposable; it must never survive as a second persistent mirror.

Candidate and `latest/` validation must prove:
...
- no unrelated `.codex` content entered the candidate or `latest/`;
- unsupported path/file forms are rejected;
- the candidate and resulting `latest/` tree are truthful managed representations of the settled source;
- a post-ingestion dry-run is equivalent;
- Git sees only expected public managed content;
- every publishable `latest/` path maps unambiguously to `AGENTS.md` or one dynamic top-level user-skill component.

**Gate 7:** `latest/` contains exactly the validated managed public payload, no second persistent mirror exists, and repeated no-op runs produce no writes or Git noise.
...
environment/config/role/path/Git/source preflight
        ↓
recursive fingerprint observation + Doc 6 settlement/mode eligibility
        ↓
build and validate private candidate from Mac Studio live source
        ↓
no difference ──► record concise no-op and stop
        ↓
promote candidate through bounded reconciliation into latest/
        ↓
latest/ validation + post-sync equivalence dry-run
        ↓
derive and validate Doc 7 artifacts + bounded README section
        ↓
conservative path-scoped Git validation
        ↓
deterministic indexed managed change set + commit message
        ↓
commit latest changes only
        ↓
SSH push
        ↓
atomic publication receipt, success state and metadata-only log
```
...
Publisher failures must stop the pipeline in place. Invalid config, missing/unreadable Codex root, invalid environment, unsafe Git state, candidate failure, rsync failure, invalid `latest/`, artifact/README mismatch, failed equivalence, commit failure or push failure must prevent later stages and produce a useful non-secret error.

Each run must log operational metadata sufficient to diagnose behaviour: timestamp, configured machine ID and role, operation attempted, change/no-change result, whether dry-run and real rsync ran, Git action/outcome, deterministic changed-component summary, final success/failure and a concise error classification. Logs must not contain managed file contents, nested managed filenames, credentials or authentication material.

The publisher must never deploy a candidate, `latest/` or GitHub state into the Mac Studio live `.codex`. Recovery remains separate and human-triggered.
...
Normal development, unattended managed-state publication and deliberate Codex Config Manager releases are different lanes.

Normal human/AI development commits may change all appropriate public project files. The unattended publisher may stage and commit only the validated transaction paths:
...
Routine managed-state publications are identified by their Git commit SHA and a deterministic `managed-state:` commit message. The unattended publisher must never create `v1`, `v2`, `v3`, SemVer or any other automatic tag or GitHub Release. A project release remains an explicit, evidence-gated operator action outside the scheduled publisher.
...
5. stage the exact `latest` pathspec so deletions are represented;
6. inspect every staged path and reject anything outside that prefix;
7. reject forbidden, malformed or unmappable indexed managed paths rather than summarize or publish them;
8. compare the published parent tree with the proposed index tree and construct one deterministic managed change set only when the staged diff is meaningful;
9. classify `AGENTS.md` and each dynamic top-level user-skill component as added, updated or removed from component presence before and after, treating nested changes inside an existing skill as a skill update;
10. conservatively represent an apparent rename as one removal and one addition rather than rely on heuristic rename meaning;
11. render a deterministic `managed-state:` subject and complete structured body with `AGENTS.md` first, skills sorted by name, action groups ordered added/updated/removed and validated `publisher: MacStudio` attribution for the initial publisher;
12. bind the base commit, proposed staged-tree identity, publisher identity, complete component list and rendered message to the pending-publication state before commit;
13. commit only that validated change set and push without force;
14. record the full commit SHA, publication outcome and complete component summary atomically for receipts/status while using a short SHA only for display;
15. log commit identity and component-level outcome without credentials, managed contents or nested managed filenames;
16. preserve a successfully created but unpushed commit after network failure;
17. recognize and safely retry only its own validated pending publication, reproducing the same summary rather than creating duplicate commits or rewriting unknown work;
18. stop for operator recovery when ahead/behind/diverged state cannot be proven safe.

The conceptual result produced once from the validated Git index is:

```text
ManagedChangeSet
├── base commit SHA
├── proposed staged-tree identity
├── validated publisher machine ID
└── components
    ├── AGENTS.md              added | updated | removed
    └── <dynamic skill name>   added | updated | removed
```

That one result feeds the Git commit subject/body, pending-publication receipt, metadata-only log and read-only status surface. Those consumers must not independently recalculate or reinterpret the change. The exact staged Git diff remains the forensic truth; the managed change set is its deterministic component-level projection.

The summary must come from the validated staged before/after trees, not raw rsync itemization or unrestricted worktree changes. It may truthfully say that `operational-modes` changed, but it must never use AI, language-model inference, keyword interpretation or managed-content inspection to claim what changed inside that skill. No-op input produces no change set, commit or push. Summary-generation failure stops publication.

Mode C must test branch-rule behaviour, authentication failure, network failure, push rejection, pre-existing dirty work, pre-existing staged work, unrelated changes, component classification, deterministic ordering/message rendering, deletion-only changes, summary-generation failure, failed retry and successful retry. The exact pending-publication storage mechanism, fixed subject-length threshold and pluralization mechanics are implementation choices, but they must be deterministic and receipt/test-backed without weakening Doc 5's component or action contract.

**Gate 9:** Automated publication cannot stage unrelated content or force-push; it creates no no-op commits or automatic tags; every valid commit carries a deterministic, complete component-level summary derived from the validated index; legitimate additions, updates and deletions are classified correctly; receipts/status share the same managed change set; and conservative failure/retry reproduces the same pending publication without duplication.
...
safe repository update
        ↓
validate `latest/` before live deployment
        ↓
checksum rsync dry-run: latest/ → consumer live managed targets
        ↓
...
- validate the `latest/` payload before touching live Codex state;
...
- use the pulled commit SHA as published-state identity but never parse human-facing commit prose as deployment authority;
...
- publisher duration/mode rendering, including initial `1m` check, `5m` settlement and `after_settle` mode;
- consumer interval rendering, including the initial `300`-second value;
...
- manually introduced `latest/skills/.system` rejected without deletion;
- `.DS_Store` ignored at repository root, private candidates, `latest/`, nested skills and consumer targets;
...
### Deterministic managed-state publication history

- exact `AGENTS.md` add/update/remove classification;
- dynamic top-level user-skill add/update/remove classification without a name allowlist;
- a nested file addition, update or removal inside an existing skill classified only as that skill being updated;
- multiple components collapsed, grouped and ordered deterministically;
- apparent rename represented conservatively as removal plus addition;
- `.system/**`, `.DS_Store`, malformed paths and unexpected indexed paths never represented as publishable components;
- byte-equivalent message and managed-change-set output from identical before/after trees;
- fixed subject-length fallback and pluralization rules;
- complete body, validated publisher attribution and no managed-content or nested-filename leakage;
- full SHA and complete component list persisted consistently across receipt and status surfaces;
- summary-generation failure stops commit and push;
- pending push retry preserves the same base/tree identity, message and component summary;
- no automatic Git tag or GitHub Release creation;
- consumer behaviour independent of commit-message parsing.
...
### Portable distribution and README

- deterministic `global-agents.zip` containing root-level `AGENTS.md` exactly equivalent to `latest/AGENTS.md`;
- one deterministic ZIP per dynamic `latest/skills/<skill-name>/` with the proven matching wrapper directory;
- artifact addition, stable replacement, bounded deletion and no-op byte identity;
- `.system/**`, `.DS_Store`, `__MACOSX/`, AppleDouble and unsafe archive-member rejection;
- bounded README view/download reconciliation that preserves all human-authored content;
- exact `latest/` ↔ artifact ↔ README membership and content equivalence;
- Mac mini consumer ignores `upload-ready/` and README distribution content.
...
- uninstall unloads/removes only generated launchd integration and preserves live Codex state, `latest/`, config, environments, logs and user data unless a separately explicit bounded removal is authorised;
- status is read-only and reports configured machine/role, environment validity, repository state, installed service state, last known result, last full publication SHA with short display form, publisher identity, deterministic changed-component summary and useful paths without secrets;
- validate is role-aware and non-mutating, checks config/environment/repository/candidate/`latest/`/managed paths and reports exact failures;
- wrong-role, missing-environment and unsafe-path cases fail conservatively for every command.

### Mac Studio consumer simulation

Simulation must create isolated temporary repositories, private candidates, `latest/` trees and fake Codex roots at runtime. It must prove initial deployment, update, no-op, deletion, invalid `latest/`, unmanaged sentinel preservation and exclusion preservation. Tests may create `.system` and `.DS_Store` conditions inside temporary directories; those artifacts must never become published managed fixtures.
...
**Gate 11:** The exact repository test command passes with a decisive summary; targeted safety, deterministic-publication and portable-distribution suites pass independently; temporary artifacts are contained and removed; no live Codex or launchd state was touched by simulation.
...
2. use the configured publisher check interval, initially `1m`/60 seconds, while the application enforces the separate `5m` settlement and publication-mode eligibility contract;
...
9. install/uninstall through explicit operator commands that never delete live Codex assets, `latest/`, config or user data by default;
10. validate the real user LaunchAgent domain, loaded state, schedule, environment resolution, log output, no-op run, meaningful authorised publication with its expected commit summary/receipt/status identity, retry behaviour and failure reporting;
...
- `uninstall` removes only the installed role integration by default and explicitly preserves live managed Codex state, `latest/`, local config, `.venv`, `.tools`, logs and runtime data;
- `status` performs no mutation and presents the configured role, environment/tool validity, Git/remote state, LaunchAgent state, last result, last publication SHA, publisher identity, changed components and relevant paths;
...
**Gate 12:** The installed Mac Studio publisher runs through launchd at the configured interval using only repository-owned runtime paths, performs no-op and authorised-change behaviour correctly, publishes through SSH with the expected deterministic commit/receipt/status identity, retries safely, logs only component-level metadata and never activates the consumer.
...
- publisher behaviour, deterministic managed-state commit semantics, Git safety and launchd operation;
- distinction between Git-SHA managed-state publications, normal project-development commits and explicitly approved SemVer project releases;
- managed change-set, pending-publication receipt, status fields and retry interpretation;
...
- GitHub commit/diff/documentation visibility model and optional documentation-only GitHub Pages boundary;
- deliberate release eligibility/procedure, with no automatic tagging by the unattended publisher;
- known Mac mini deferrals;
- bounded Mac mini handoff.

The root `README.md` should become the concise public entry point while preserving the legitimate existing remote history. It must expose the Doc 7 global `AGENTS.md` view/download links and dynamically reconciled per-skill downloads through a bounded managed section. Bootstrap documents remain preserved. Discovery and plan documents must be marked with truthful lifecycle status once implementation milestones are actually validated; they must not be left implying that future work remains current after it is complete.
...
- only the exact validated `latest/`, Doc 7 artifact and bounded README paths are writable by unattended publication;
- `latest/` truthfully represents the current validated managed Mac Studio snapshot and is not an authoring source;
- the public managed-state history carries deterministic component-level commit context and immutable SHA identity without automatic release tags;
- receipt, status and Git commit evidence agree for the last authorised Mac Studio publication;
...
5. validate local paths, permissions, filesystem semantics and `latest/`;
6. install only the consumer LaunchAgent;
7. prove safe fast-forward Git updates and invalid-state failures;
8. prove that pulled commit SHA identity is observable while commit-message prose is not deployment authority;
9. prove bounded deployment, no-op and legitimate deletion against the real Mac mini `.codex`;
10. prove `.system/**`, `.DS_Store` and unrelated `.codex` content remain untouched;
11. prove real launchd scheduling, deterministic runtime resolution, logging and failure behaviour;
12. complete the Mac Studio → GitHub → Mac mini end-to-end proof;
13. contribute truthful consumer-specific fixes, tests and documentation to the same canonical repository within the established extension boundary.

A Mac mini discovery that changes source-of-truth direction, managed scope, private-candidate or `latest/` semantics, deletion policy, publisher behaviour, public config architecture, repository topology or safety boundaries is not a local fix. It must be escalated as an explicit architecture decision.
...
| Managed component mapping and action classification | Phases 6, 7 and 9 | One validated managed change set derived from the indexed before/after trees |
| Commit grammar, ordering and subject fallback | Phase 9 | Deterministic renderer, receipt binding and targeted tests |
| Publication SHA, receipt and status agreement | Phases 9, 11 and 12 | Repository-local tests plus real Mac Studio launchd evidence |
| Managed publication versus project release separation | Phases 1, 9 and 13 | No unattended tags; documented explicit evidence-gated release procedure |
...
The Mac Studio phase is complete only when Gates 0–14 pass, the public repository contains the complete reusable system, the publisher operates headlessly through its repository-owned environment with deterministic component-level publication history, consumer simulation is safe and comprehensive, and permanent documentation plus the bounded Mac mini handoff are published.
