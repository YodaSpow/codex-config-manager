...
The unattended Mac Studio publisher may stage and commit only one coherent validated transaction across:
...
Only canonical `latest/` changes contribute semantic components. Doc 7 artifacts and the bounded README section are deterministic projections committed atomically with that canonical state. Each successful non-no-op publication receives a deterministic component summary and is identified immutably by its Git commit SHA.
...
checksum-aware rsync dry-run
        ↓
controlled private-candidate validation and latest/ update
        ↓
latest/ validation and post-sync equivalence
        ↓
derive and validate Doc 7 artifacts + bounded README section
        ↓
stage the exact approved transaction pathspecs
        ↓
reject every indexed path outside the approved transaction
        ↓
inspect canonical latest/ in the staged before/after trees
        ↓
...
The summary must be generated only after `latest/` and derived-state validation plus exact path-scoped Git staging. It must never be generated from unrestricted worktree changes or derived artifact bytes.
...
For skills, `<skill-name>` is the immediate child directory beneath `latest/skills/`. That directory is the semantic skill identity. Repeated internal filenames such as `SKILL.md`, references, scripts or assets are implementation detail and do not belong in the concise publication summary.
...
Only canonical `latest/` content that has already passed the managed ingestion, candidate and snapshot contracts may participate in summary generation.
...
Summary generation is not a late exclusion mechanism. Excluded content must never enter a private candidate, `latest/`, derived artifact or Git index in the first place.
...
- stage the exact validated `latest/**`, Doc 7 artifact and bounded README pathspecs and prove no other indexed path can enter an unattended commit;
- derive publication context only from canonical `latest/` in the validated Git-index before/after trees;
- validate that every committed artifact and README entry corresponds exactly to the same canonical change set without reporting those projections as separate semantic components;
...
- ▶ Future Mode C must implement the deterministic publication summary and prove it against repository-local tests and real Git-index behaviour over canonical `latest/`.
- ✅ Docs 3, 7 and 8 now place this record within the active implementation and distribution architecture.
