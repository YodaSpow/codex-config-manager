...
- ✅ Canonical `latest/` remains the unpacked repository representation used by the Mac Studio publisher and Mac mini consumer.
...
- ✅ GitHub direct download uses absolute `raw.githubusercontent.com` links derived from the publisher's already validated GitHub remote identity and configured branch. The earlier relative `?raw=1` approach is rejected because a live click inside GitHub's rendered README produced an error page even though detached HTTP navigation could follow its redirects.
...
Mac Studio authoritative ~/.codex managed state
        ↓
settled and validated latest state
        ↓
...
```text
canonical unpacked latest/ snapshot
        ↓
...
`latest/` is canonical. `upload-ready/` and the README download list are deterministic projections of that canonical state.

Portable artifacts and README entries must never:

- become authoring sources;
- determine which managed files or skills exist;
- repair or repopulate `latest/`;
- preserve a skill absent from canonical `latest/`;
- conceal a mismatch with the same commit's canonical snapshot;
- be deployed by the Mac mini consumer;
- introduce renamed or semantically edited copies of managed content.

If `latest/`, artifacts and README membership disagree, publication must fail. The publisher repairs derived state only by rebuilding it from validated `latest/`; it never changes `latest/` to match an artifact or README entry.
...
The three named skills are the currently observed examples, not an allowlist. The implementation must discover all eligible immediate child directories beneath `latest/skills/` dynamically.
...
If canonical `latest/AGENTS.md` is absent, `global-agents.zip` and its README download entry must be absent. The README view entry must not point knowingly to a nonexistent canonical file.
...
The artifact basename and wrapper directory must match the `latest/skills/` parent-directory name exactly. The publisher must not infer names from `SKILL.md` display metadata, convert names to titles, apply a branding prefix or hard-code the currently observed skills.

The one-to-one membership invariant is:

```text
set(latest/skills/<eligible immediate child directory names>)
...
Doc 1 remains authoritative: exclusions happen at ingestion, before a managed candidate or canonical `latest/` snapshot exists.
...
- [Download chat-handoff](https://raw.githubusercontent.com/<derived-owner>/<derived-repository>/<configured-branch>/upload-ready/skills/chat-handoff.zip)
- [Download operational-modes](https://raw.githubusercontent.com/<derived-owner>/<derived-repository>/<configured-branch>/upload-ready/skills/operational-modes.zip)
- [Download semantic-compression](https://raw.githubusercontent.com/<derived-owner>/<derived-repository>/<configured-branch>/upload-ready/skills/semantic-compression.zip)
```
...
On GitHub, the ordinary relative `.md` link opens the normal Markdown file page, which provides rendered preview and raw/source access. Every ZIP target uses an absolute `raw.githubusercontent.com` URL. The publisher fixes only GitHub's raw-content host; it derives owner and repository from the already validated Git remote and takes the branch from configuration before appending the validated, deterministically encoded artifact path. The implementation does not hard-code project identity. The visible labels and artifact paths remain stable.
...
The same canonical `latest/` input and packaging-contract version must produce byte-identical ZIP bytes. Otherwise a no-op publisher run could create meaningless binary Git churn.
...
1. `global-agents.zip` contains exactly one root member named `AGENTS.md` whose bytes equal `latest/AGENTS.md`.
2. Every skill ZIP basename equals one eligible `latest/skills/` directory name.
3. Every skill ZIP contains exactly one matching wrapper directory and the complete eligible canonical subtree beneath it.
...
Doc 6 eligibility remains upstream. No candidate promotion, `latest/`, ZIP or README mutation occurs for merely observed, unsettled, paused, scheduled-held or throttled-held source state.
...
Doc 6 publication mode becomes eligible
        ↓
build and validate private candidate from settled source
        ↓
bounded reconciliation updates latest/ from candidate
        ↓
canonical latest/ validation succeeds
        ↓
...
stage exact canonical, artifact and README pathspecs
        ↓
derive Doc 5 ManagedChangeSet from canonical latest/ paths in the Git index
        ↓
...
- A newly added canonical skill creates exactly one corresponding ZIP and README entry.
- A changed canonical skill atomically replaces its same stable ZIP; no numbered or timestamped copy remains.
- A removed canonical skill removes only its corresponding managed ZIP and README entry.
...
The canonical managed path remains:

```text
latest/**
```

The reconciled Docs 3, 5 and 8 unattended transaction allowlist covers:
...
Dynamic ZIP membership is acceptable only because every allowed artifact must correspond one-to-one with a validated top-level `latest/` skill. It is not permission for:
...
The application must reject unexpected Git paths or unrelated working-tree changes rather than staging them. Docs 3, 5 and 8 now reconcile the paths the unattended publisher may mutate and commit; Mode C must implement that exact allowlist.
...
```text
latest/AGENTS.md
latest/skills/<dynamic user-skill content>
```
...
## Reconciled impact on existing documents

### Doc 3

The protected Doc 8 reconciliation establishes:
...
| Publisher phases | Build artifacts only after eligible canonical `latest/` succeeds; validate and commit all projections atomically |
...
Doc 5 preserves canonical paths as semantic authority while allowing exact derived artifacts and bounded README changes in the same commit. Push retry retains the identical artifact set associated with the existing commit SHA.

### Doc 6

Doc 6 remains the eligibility layer. `paused`, scheduled-held and throttled-held state performs detection/reporting only and must not mutate a candidate, `latest/`, artifacts or README.
...
6. Exact Git pathspec validation for the dynamic but `latest/`-derived skill ZIP set.
...
- ✅ The aggregate ZIP proposal is replaced by one stable ZIP per dynamically discovered `latest/` user skill.
